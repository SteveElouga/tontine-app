...
- **En cours** : story **Simulation** sur `feat/TON-14-simulation`. Deux volets « et si… » : Épargne (montant + mois → taux, intérêt, total à la clôture) et Prêt (montant + mois + remboursement → mois de dette, majoration, total). Calcul **côté serveur** (queries `simulerEpargne`/`simulerPret` → moteur de domaine, jamais dupliqué), résultat instantané. Nav « Simulation » activée. **Vérifié** : `ngc` exit 0. (TON-12 sélecteur de cycle + TON-13 historique : mergés via PR #13.)
